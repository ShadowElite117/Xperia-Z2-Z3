 ...
 * Copyright (C) 2016 Tristan Marsell (tristan.marsell@t-online.de). All rights reserved
 * To get worok om Z3 some things needed to be bridged and changed 
 *
		...
		uint32_t param_id, uint8_t *param, uint32_t param_len)
{
	...
	rc = adm_send_params_v5(port_id, ahc_copp_idx, params_value,
			pkt_len);
	if (rc)
		...
		uint32_t *param_id, uint32_t *module_id)
{
	...
	case AHC_MBDRC_REG_ENABLE:
		...
		*param_len = sizeof(struct audproc_common_enable);
		*param_id = AUDPROC_PARAM_ID_ENABLE;
		*module_id = ASM_MODULE_ID_MBDRCV2;
		break;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	 ...
	 */
	rc = adm_ahc_get_params(AHC_PORT_ID,
			ahc_copp_idx,
			module_id,
			param_id,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	case AHC_MBDRC_REG_ENABLE: {
		...
		pr_debug("%s: AHC_MBDRC_REG_ENABLE recvd val: %u\n",
				__func__, param.enable);
		rc = iir_tf_send_params(AHC_PORT_ID, ASM_MODULE_ID_MBDRCV2,
				AUDPROC_PARAM_ID_ENABLE,
				(uint8_t *)&param, sizeof(param));
		break;
	}
